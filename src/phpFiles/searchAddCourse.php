...
<body>
    ...
    <div class="containerForm my-5">
        ...
        if (count($errorMsg)>0) {
            foreach ($errorMsg as  $error) {
                echo "<div class='alert alert-danger alert-dismissible fade show' role='alert'>
                        $error
                        <button onclick='history.back(-2)' type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                    </div>";
            }
        }
        ?>
        ...
        <?php
        if (isset($_GET['year']) && isset($_GET['program']) && isset($_GET['courseCode']) && isset($_GET['courseRev']) && isset($_GET['duration']) && isset($_GET['courseName']) && isset($_GET['courseDescription'])) {
            $year = $_GET['year'];
            $program = $_GET['program'];
            // added the program to the course code
            $courseCode = $_GET['courseCode'] . '_' . strtoupper($program);
            // Added 1 to the course rev so its uni
            $courseRev = $_GET['courseRev']+1;
        ...
        <form action="addCourses.php" method="post">
            ...
            <div class="form-group">
                <input type="text" class="form-control" name="courseCode" placeholder="Course Code: COURSE_CODE" value="<?php echo $courseCode; ?>" readonly>
            </div>
            <div class="form-group">
                <input type="number" class="form-control" name="courseRev" placeholder="Course Rev: 1 - 3" value="<?php echo $courseRev; ?>" readonly>
            </div>
            <div class="form-group">
                <input type="number" class="form-control" name="duration" placeholder="Duration (Hours): " value="<?php echo $duration; ?>" readonly>
            </div>
            <div class="form-group">
                <input type="text" class="form-control" name="courseName" placeholder="Course Name:" value="<?php echo $courseName; ?>" readonly>
            </div>
            <div class="form-group">
                <input type="text" class="form-control" name="courseDescription" placeholder="Course Description: " value="<?php echo $courseDescription; ?>" readonly>
            </div>
            <div class="form-group">
